  ...
  try {
    ...
    records.forEach((record, index) => {
      ...
      if (record.url && record.size && record.size < 500 * 1024) {
        ...
        pdf.text(`[Attached] ${recordType}`, 160, yPosition);
        pdf.setTextColor(...textColor);
      } else {
...
/**
 * Get a readable file type from record data
 */
function getReadableFileType(record) {
  // Use mimeType first, then fallback to type
  if (record.mimeType) {
    switch (record.mimeType) {
      ...
      case 'application/xml':
        return 'XML Document';
      default: {
        // Try to extract main type
        const mainType = record.mimeType.split('/')[0];
        if (mainType === 'image') return 'Image';
        if (mainType === 'application') return 'Document';
        if (mainType === 'text') return 'Text File';
